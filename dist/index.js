...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        const rule = yield lb_rule_mgmt_1.configureLoadBalancerRule({
            tags: core_1.getInput("tags"),
            actions: core_1.getInput("actions"),
            listener: core_1.getInput("listener"),
            priority: core_1.getInput("priority"),
            conditions: core_1.getInput("conditions"),
        });
        if (!rule)
            throw new Error("Failed to apply changes");
        core_1.info(`Rule ARN: ${rule.RuleArn}`);
        core_1.setOutput("rule-arn", rule.RuleArn);
        return 0;
    });
}
exports.run = run;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.configureLoadBalancerRule = void 0;
...
function configureLoadBalancerRule(input) {
    return __awaiter(this, void 0, void 0, function* () {
        const { tags, actions, listener, priority, conditions } = input;
        const ruleData = {
            ListenerArn: listener,
            Priority: yield processPriority(priority, listener),
        };
        if (tags)
            ruleData.Tags = processTags(tags);
        if (actions)
            ruleData.Actions = processActions(actions);
        if (conditions)
            ruleData.Conditions = processConditions(conditions);
        const client = getClient();
        const { Rules } = yield client.createRule(ruleData).promise();
        return Rules === null || Rules === void 0 ? void 0 : Rules.pop();
    });
}
exports.configureLoadBalancerRule = configureLoadBalancerRule;
